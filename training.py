...
class PyramidGAN:
    ...
    def __init__(self, Generator, Discriminator, opt):
        ...
        if self.opt.mode != 'train':
            self.Gs, self.Zs, _, _, self.NoiseAmp = load_trained_pyramid(self.opt)
    ...
    def train_epoch(self, netG, netD, optimizerG, optimizerD, real, style, m_noise, m_image, epoch, z_opt, z_prev, opt):
        ...
        for j in range(opt.Dsteps):
            ...
            # If not very first epoch, just generate previous image
                ...
                prev = draw_concat(self.Gs, self.Zs, self.reals, self.styles, self.NoiseAmp, self.in_s, 'rand', m_noise, m_image, opt)
                prev = m_image(prev)
        ...
        for j in range(opt.Gsteps):
            ...
            if opt.rec_weight != 0:
                ...
                rec_loss = opt.rec_weight * scale_weight[len(self.Gs)] * loss(netG(noise.detach(), prev, m_real), real)
            ...
            style_loss = opt.alpha * (1 - scale_weight[len(self.Gs)]) * adain_style_loss(style, fake_style, self.vgg, opt.style_layers) 
            style_loss.backward()

            optimizerG.step()

        if epoch % opt.niter_update == 0 or epoch == (opt.niter-1):
            plt.imsave('%s/fake_style_sample.png' % (opt.outf), convert_image_np(fake_style.detach()), vmin=0, vmax=1)
            plt.imsave('%s/fake_real_sample.png' % (opt.outf), convert_image_np(fake.detach()), vmin=0, vmax=1)

            plt.imsave('%s/G(z_opt).png' % (opt.outf), convert_image_np(netG(Z_opt.detach(), z_prev, m_real).detach()), vmin=0, vmax=1)
